...
if (isset($_POST['submit'])) {
    ...
    if (move_uploaded_file($tempImage, $imagePath) && move_uploaded_file($tempAudio, $audioPath)) {
        $insertQuery = "INSERT INTO music (name, description, duration, year, image, audio, artist, album) 
                        VALUES ('$name', '$description', '$duration', '$year', '$image', '$audio', '$artist', '$album')";

        if (mysqli_query($connect, $insertQuery)) {
            echo "<script>alert('Music Added Successfully'); window.location.href='showmusic.php';</script>";
        } else {
            echo "<script>alert('Failed to Add Music');</script>";
        }
    } else {
        echo "<script>alert('Image or Audio Upload Failed');</script>";
    }
}
?>
...
<div class="container d-flex justify-content-center align-items-center" style="min-height: 100vh;">
    <div class="gradient-border" style="width: 520px;">
        <div class="inner-card">
            <h3 class="text-center mb-4">Add New Music</h3>
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="text-center">
                    <button type="submit" name="submit" class="purple">Add Music</button>
                </div>
            </form>
        </div>
    </div>
